...
describe('TCPStream', function () {
  ...
  it('should transform a pcap-packet stream to a buffer stream', function () {
  ...
  it('should do nothing when the packet is not a tcp packet', function () {
    when(pcapPacket.payload).thenReturn(instance(ethernetPacket));
    when(ethernetPacket.payload).thenReturn(instance(ipv4));
  ...
  it('should do nothing when the packet has no data', function () {
    when(pcapPacket.payload).thenReturn(instance(ethernetPacket));
    when(ethernetPacket.payload).thenReturn(instance(ipv4));
    when(ipv4.payload).thenReturn(instance(tcp));
  ...
  it('should transform a pcap-packet stream with mixed packets to a buffer stream', function () {
    ...
    const ret1 = stream.read() as Buffer;
    const ret2 = stream.read() as Buffer;

    expect(ret1).to.be.deep.equal(Buffer.from('0500000002', 'hex'));
    expect(ret2).to.be.deep.equal(Buffer.from('0500000003', 'hex'));
  });

  it('should transform a pcap-packet stream with several packets to a buffer stream', function () {
    const tcp2: TCP = imock();
