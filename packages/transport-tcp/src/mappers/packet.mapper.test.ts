...
describe('PacketMapper', function () {
  ...
  beforeEach(function () {
    payloadMapper = imock();
    packetMapper = new PacketMapper(instance(payloadMapper));
  });
  ...
  describe('#toDomain()', function () {
    it('should map a buffer to a packet with payload', function () {
      ...
      when(payloadMapper.toDomain(anything(), anything())).thenReturn(payload);
      ...
      const [payloadBuffer, opcode] = capture(payloadMapper.toDomain<'DEVICE_GETTIME_RSP'>).first();

      expect(opcode.equals(OPCode.fromName('DEVICE_GETTIME_RSP'))).to.be.true;
      expect(payloadBuffer.toString('hex')).to.equal('08001a090893afeefd0510901c');
    });
    ...
    it('should map a buffer to a packet without payload', function () {
      ...
      expect(opcode.equals(OPCode.fromName('DEVICE_GETTIME_RSP'))).to.be.true;
      expect(payloadBuffer.toString('hex')).to.equal('');
    });
  ...
  describe('#toBuffer()', function () {
    it('should map a packet to a buffer', function () {
      const buffer = Buffer.from('08001a090893afeefd0510901c', 'hex');
      ...
      when(payloadMapper.fromDomain(anything())).thenReturn(buffer);

      const ret = packetMapper.fromDomain(packet);

      expect(ret.toString('hex')).to.equal(
